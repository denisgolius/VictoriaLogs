...
* BUGFIX: [querying API](https://docs.victoriametrics.com/victorialogs/querying/): limit the number of returned points for [`/select/logsql/hits`](https://docs.victoriametrics.com/victorialogs/querying/#querying-hits-stats) and [`/select/logsql/stats_query_range`](https://docs.victoriametrics.com/victorialogs/querying/#querying-log-range-stats) with `-search.maxPointsPerTimeseries=30000` based on the effective query time range. These endpoints now require bounded time range provided either via HTTP `start` / `end` args or via LogsQL [`_time` filter](https://docs.victoriametrics.com/victorialogs/logsql/#time-filter). This helps prevent excessive CPU and memory usage on graph-oriented endpoints. See [#1322](https://github.com/VictoriaMetrics/VictoriaLogs/issues/1322).
